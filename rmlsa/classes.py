...
def create_thumbnail(img_model):
    ...
    if django_type == 'image/jpeg':
        ...
        raise TypeError('{} File type not supported. Please use .jpg, .jpeg, .gif or .png.')
    ...
    suf = SimpleUploadedFile(os.path.split(img_model.image.name)[-1], temp_handle.read(), content_type=django_type)

    # Save SimpleUploadedFile into image field
    try:
        img_model.thumbnail.save('{}_thumbnail.{}'.format(os.path.splitext(suf.name)[0], ext), suf, save=False)
